...
<table class="nmtable" >
	...
	<?php 
		...
		$ros = $con->query("select* from pmsadmin");
		foreach ($ros as $ro) {?>
			<tr><td>
			<?php
			$tk = $ro['adminid'];
			if(strlen($ro['adminid']) != 0){
				$n = $ro['adminid'];
				echo $n;
			}?><td/>
			<?php
			...
			<a href='adminlist.php?d=<?php echo $tk?>'>delete</a></td>
			</tr>
	<?php };}
...
<?php 
		...
		$ros = $statement->fetchAll();*/
                $ros = $con->query("select* from pmsadmin where fname like '%$search%' or lname like '%$search%' or username like '%$search%'");
                if(!empty($ros)){
		foreach ($ros as $ro) {?>
			<tr><td>
			<?php
