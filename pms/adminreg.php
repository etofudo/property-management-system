...
<?php
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		if($_FILES['im']['error'] > 0){
			...
			$un = $_POST['usn'];
			$x = 0;
			$sql = "select adminid from pmsadmin where username='$un'";
			$a = $con->query($sql);
			if($a->num_rows===0){
				...
				$psw = md5($_POST['pasw']);
				$ros = $con->query("select lastadminid, noofaprofilepics from general");
					foreach ($ros as $ro) {
						$ll = $ro['lastadminid'] + 1;
						$npp = $ro['noofaprofilepics'] + 1;
					}
				$pim = "adminprofilepics/"."$ll"."_profilepics_"."$npp";
				move_uploaded_file($_FILES['im']['tmp_name'],"$pim".'.png');
				$sql = "INSERT INTO pmsadmin (username, password, fname, lname, gender, img) VALUES ('$un', '$psw','$fname','$lname','$gender','$pim')";
				if($con->query($sql) === TRUE){
					...
					$sql = "update general set lastadminid='$ll', noofaprofilepics='$npp'";
					if($con->query($sql) === TRUE){
